...
function removeFirstOccurrences(str, value) {
  ...
  return substrIndex >= 0
    ? str
        .substring(0, substrIndex)
        .concat(str.substring(substrIndex + value.length, str.length))
    : str;
}
...
function removeLastOccurrences(str, value) {
  const substrLastIndex = str.lastIndexOf(value);
  return substrLastIndex >= 0
    ? str
        .substring(0, substrLastIndex)
        .concat(str.substring(substrLastIndex + value.length, str.length))
    : str;
}
...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replaceAll(/[^a-z]/gi, '');
...
function encodeToRot13(str) {
  ...
  const encryptLetter = (char) => {
    const beginLetterCode = (
      char.toLowerCase() === char ? 'a' : 'A'
    ).charCodeAt(0);
    return String.fromCharCode(
      ((char.charCodeAt(0) - beginLetterCode + SHIFT) % 26) + beginLetterCode
    );
  };
  return str
    .split('')
    .map((char) => (char.match(/[a-z]/gi) ? encryptLetter(char) : char))
    .join('');
}
